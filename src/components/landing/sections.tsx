  ...
  return (
    <section aria-label="Intégrations" className="relative py-14 sm:py-20">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        ...
        <Reveal delay={120} effect="rise3d">
          <ul className="mt-10 flex flex-wrap items-center justify-center gap-2.5 sm:gap-3">
            {copy.trust.tools.map((t) => (
              <li
                key={t}
                className="rounded-full border border-white/[0.08] bg-white/[0.03] px-5 py-2.5 text-[13.5px] font-medium text-[#cfd6cd] transition hover:-translate-y-0.5 hover:border-[var(--accent)]/30 hover:text-[var(--fg)]"
              >
                {t}
              </li>
            ))}
          </ul>
          <p className="mt-7 text-center text-[12.5px] text-[var(--fg-muted)]">{copy.ui.euHosting}</p>
        </Reveal>
      </div>
    </section>
  );
  ...
  return (
    <section aria-labelledby="problem-title" className="relative py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        ...
        <div className="mt-12 grid gap-4 sm:gap-5 md:grid-cols-3" style={{ perspective: 1200 }}>
          {copy.problem.items.map((item, i) => (
            <Reveal key={item.title} delay={i * 110} effect="rise3d" className="h-full">
              <Tilt className="h-full rounded-3xl">
                <article className="h-full rounded-3xl border border-white/[0.07] bg-gradient-to-b from-white/[0.04] to-transparent p-7 transition-colors hover:border-[var(--accent)]/25">
        ...
        <ol className="relative mt-14 grid gap-3 md:grid-cols-5 md:gap-4">
          ...
          {copy.solution.steps.map((s, i) => (
            <li key={s.title} className="relative">
              <Reveal delay={i * 120} effect="rise3d" className="h-full">
                <Tilt max={5} className="h-full rounded-2xl">
                  <div className="flex h-full flex-col items-start gap-3 rounded-2xl border border-white/[0.07] bg-[#0b0d0a]/80 p-5 md:items-center md:text-center">
                    <span
                      aria-hidden
      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        ...
        <div className="mt-12 grid gap-4 sm:gap-5 md:grid-cols-2">
          {copy.offers.items.map((o, i) => (
            <Reveal key={o.num} delay={(i % 2) * 110} effect="rise3d" className="h-full">
              <Tilt className="h-full rounded-3xl">
                <article className="group relative h-full overflow-hidden rounded-3xl border border-white/[0.07] bg-gradient-to-b from-white/[0.04] to-transparent p-7 transition-colors hover:border-[var(--accent)]/25 sm:p-9">
                  <span
                    aria-hidden
                  ...
                  <p className="relative mt-3.5 text-[14.5px] leading-relaxed text-[#aab3a9]">
                    {o.body}
                  </p>
                </article>
              </Tilt>
        ...
        <ol className="relative mt-14 space-y-0 border-l border-white/[0.08] pl-8 sm:pl-12">
          {copy.process.steps.map((s, i) => (
            <li key={s.num} className="relative pb-11 last:pb-0">
              <Reveal delay={i * 90} effect="rise3d">
                <span
                  aria-hidden
